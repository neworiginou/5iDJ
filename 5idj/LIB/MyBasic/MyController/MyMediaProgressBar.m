...
//  GamePlayerDemo
//
//  Created by Xuzhanya on 14-2-26.
...
- (void)_updateThumb
{
    if (_thumbImageView) {
        ...
        if (self.thumbImageNormal) {
            ...
            if (_thumbImageView.isHighlighted) _thumbImageView.layer.shadowOpacity = 1.f;
        }
    }
}
...
#define TrackStartX(_thumbWidth)    \
    (_thumbWidth * 0.5f + TouchPadding)

#define TrackEndX(_thumbWidth)      \
    (CGRectGetWidth(self.bounds) - _thumbWidth * 0.5f - TouchPadding)
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    if (_dragging) {
        ...
        self.glowTrackShapeLayer.strokeEnd = ValueToProgress(newValue);
        
        //发送事件
        if(_value != newValue && _continuous){
            _value = newValue;
